  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sand-200/60 font-sans mt-1">Phone health and last heartbeat</p>
      </div>
      ...
      {offline.length > 0 && (
        <div className="mb-4 rounded-md border border-clay-500/40 bg-clay-600/15 px-4 py-3 text-sm font-sans">
          ...
          <p className="text-sand-200/70 mt-1">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {(data ?? []).map((d) => (
          ...
          >
            ...
            <p className="mt-2 text-sm text-sand-200/60 font-sans">
              {d.walletNumber?.label} ·{" "}
              {d.walletNumber?.provider ? providerLabel(d.walletNumber.provider) : "—"}
            </p>
            <p className="font-mono text-xs text-sand-200/50 mt-1">{d.walletNumber?.msisdn}</p>
            <p className="mt-3 text-xs font-mono text-sand-200/55">
              Last seen: {d.lastSeenAt ? new Date(d.lastSeenAt).toLocaleString() : "never"}
            </p>
          </div>
        ))}
        {data?.length === 0 && (
          <p className="text-sand-200/40 font-sans col-span-full">No devices registered</p>
        )}
      </div>
    </div>
  );
