...
2. Leave **Root Directory** empty (repo root). `vercel.json` sets build/output.
...
4. Deploy.
5. Copy the Vercel URL → set Render `CORS_ORIGIN` to that exact origin (no trailing slash) → **Manual Deploy** on Render if needed.
